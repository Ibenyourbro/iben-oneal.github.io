...
var reverse = function(string,outputStr = '') {
  ...
  return reverse(string.substring(0,string.length - 1),outputStr)
};

// 10. Write a function that determines if a string is a palindrome.
var palindrome = function(newString) {
  str = newString.toLowerCase().replace(/\s+/g, "");
...
var rMap = function(array, callback,output = [],arrC = [...array]) {
  //base
  if(arrC.length === 0){
    return output;
  }
  //recursion
  output.push(callback(arrC[0]));
  arrC.shift();
  return rMap(array,callback,output,arrC);
};
...
var capitalizeWords = function(input,output=[]) {
    ...
    input.shift();
    return(capitalizeWords(input,output))
...
var capitalizeFirst = function(array,output=[]) {
  ...
  array.shift();
  return capitalizeFirst(array,output);
};
